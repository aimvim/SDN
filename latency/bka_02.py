...
def early_stopping_check(fitness_history, patience, min_improvement):
...
# ====================== 优化的黑翅鸢算法 ======================
def bka_optimization_enhanced():
    # 算法参数
    num_kites = 50  # 黑翅鸢数量
    max_iterations = 150  # 最大迭代次数
    elite_ratio = 0.1  # 精英保留比例
    patience = 35  # 早停耐心值
    min_improvement = 1e-9  # 最小改善阈值
    ...
    plt.title('Enhanced BKA Algorithm Convergence Curve')
    ...
    # 子图2：适应度分布
    plt.subplot(2, 1, 2)
    fitness_values = [kite['fitness'] for kite in kites]
    plt.hist(fitness_values, bins=20, alpha=0.7, color='skyblue', edgecolor='black')
    plt.axvline(best_kite['fitness'], color='red', linestyle='--', linewidth=2, label=f'Best: {best_kite["fitness"]:.6f}')
    ...
    plt.tight_layout()
    plt.savefig('bka_02_convergence.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def save_results_to_file_bka_enhanced(filename, toponame, nodes_num, links_num, controller_num, avg_latency,
                         worst_latency, load, avg_compute_time, iterations_used):
    '''
    保存增强版BKA结果到文件
    :param filename: 要存入数据的文件名
    :param iterations_used: 实际使用的迭代次数
    '''
    toponame = toponame.replace(".txt", "")
    with open(filename, 'a') as f:
        f.write(f"bka_enhanced_{toponame}_{controller_num}{{\n")
...
# ====================== 运行优化 ======================
print(f"开始使用增强版黑翅鸢算法求解SDN控制器放置问题...")
...
# 运行优化算法
best_solution, fitness_history = bka_optimization_enhanced()
...
print(f"收敛曲线已保存为 'bka_02_convergence.png'")

# 保存结果到文件
result_file = os.path.join(os.getcwd(), 'bka_02_results.txt')
with open(result_file, 'a') as f:
    f.write(f"=== Enhanced BKA Algorithm Results ===\n")
    ...
    f.write(f"收敛曲线已保存为 'bka_02_convergence.png'\n")
